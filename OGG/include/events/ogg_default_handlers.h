...
{
    get_application()->is_top_level = ogg_true;
...
static void ogg_default_handler(OGG_MOUSE_DRAG_EVENT)(int x, int y)
{
    ...
    ogg_send_event(ogg_active_form(), OGG_MOUSE_DRAG_EVENT, 
        x - get_application()->last_drag_pc.x, y - get_application()->last_drag_pc.y);
}
...
{
    if (get_application()->focused_component == self) {
        get_application()->focused_component = 0;
    }
    if (get_application()->current_component == self) {
        ogg_send_event(get_application()->current_component, OGG_LOSE_FOCUS_EVENT);
        get_application()->current_component = ogg_active_form();
    }
    return ogg_true;
}
...
{
    return is_child_of(get_application()->current_component, self);
}
...
static def_checker(OGG_MOUSE_EVENT)
{
    ...
    {
        if (get_application()->is_top_level) {
            get_application()->is_top_level = ogg_false;
            if (get_application()->current_component != self) {
                if (get_application()->current_component != 0)
                    ogg_send_event(get_application()->current_component, OGG_LOSE_FOCUS_EVENT);
                get_application()->current_component = self;
                ogg_send_event(self, OGG_FOCUS_EVENT);
            }
            if (get_application()->focused_component != self) {
                if (get_application()->focused_component)
                    ogg_send_event(get_application()->focused_component, OGG_MOUSE_LEAVE_EVENT);
                get_application()->focused_component = self;
                ogg_send_event(self, OGG_MOUSE_ENTER_EVENT);
            }
            if (get_application()->mouse_state != state) {
                get_application()->mouse_state = state;
...
static def_checker(OGG_MOUSE_DRAG_EVENT)
{
    ogg_anchor anchor;
    get_component_real_coord_anchor(self, &anchor);
    int x = get_application()->last_drag_pc.x + dx, y = get_application()->last_drag_pc.y + dy;
    if (x >= anchor.coord.left && x <= anchor.coord.right &&
            y >= anchor.coord.top && y <= anchor.coord.bottom)
    {
        if (get_application()->is_top_level) {
        ...
        if (!get_application()->is_dragging) {
            if (get_application()->mouse_state == GLUT_DOWN) {
                get_application()->is_dragging = ogg_true;
                get_application()->last_drag_pc.x += x;
                get_application()->last_drag_pc.y += y;
                ogg_send_event(self, OGG_MOUSE_DRAG_BEGIN_EVENT);
            }
        } else {
            if (get_application()->mouse_state == GLUT_UP) {
                get_application()->is_dragging = ogg_false;
                ogg_send_event(self, OGG_MOUSE_DRAG_END_EVENT);
                get_application()->last_drag_pc.x = get_application()->last_drag_pc.y = 0;
            }
        }
        return ogg_true;
    }
